...
int main(int argc, const char * const argv[])
{
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({m, n}), input2, MemoryDimensions({m, n}), expected);
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({1, n}), input2, MemoryDimensions({m, n}), expected);
    std::cout << "Row broadcasting (B) | Error: " << error << std::endl;
    totalError += error;

    error = testVectorMultNode(MemoryDimensions({1, n}), input2, MemoryDimensions({m, n}), input1, MemoryDimensions({m, n}), expected);
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({m, 1}), input2, MemoryDimensions({m, n}), expected);
    std::cout << "Column broadcasting (B) | Error: " << error << std::endl;
    totalError += error;

    error = testVectorMultNode(MemoryDimensions({m, 1}), input2, MemoryDimensions({m, n}), input1, MemoryDimensions({m, n}), expected);
    std::cout << "Column broadcasting (B) | Error: " << error << std::endl;
    totalError += error;
