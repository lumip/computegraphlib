...
float testVectorMultNode(MemoryDimensions input1Dim, InputDataBuffer& input1, MemoryDimensions input2Dim, InputDataBuffer& input2, ConstDataBuffer& expected)
{
    ...
    // get output (pointer to working memory of VectorMultNode which holds the computation result)
    DataBuffer result(compilationMemoryMap.GetNodeMemoryDimensions(&testMultNode).size());
...
int main(int argc, const char * const argv[])
{
    ...
    // similar sized data
    DataBuffer input1 { 1,2,3,4, 2,2,2,2, 0,0,0,7, 1,0,-1,0, -1,-3,-5,-7 };
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({m, n}), input2, expected);
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({1, n}), input2, expected);
    std::cout << "Row broadcasting (B) | Error: " << error << std::endl;
    totalError += error;

    error = testVectorMultNode(MemoryDimensions({1, n}), input2, MemoryDimensions({m, n}), input1, expected);
    ...
    error = testVectorMultNode(MemoryDimensions({m, n}), input1, MemoryDimensions({m, 1}), input2, expected);
    std::cout << "Column broadcasting (B) | Error: " << error << std::endl;
    totalError += error;

    error = testVectorMultNode(MemoryDimensions({m, 1}), input2, MemoryDimensions({m, n}), input1, expected);
    std::cout << "Column broadcasting (B) | Error: " << error << std::endl;
    totalError += error;
